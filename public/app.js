...
let selected=null, settings={difficulty:'normal',sound:true,crt:true,visual:'glow',controlMode:'desktop'};
...
function renderCards(){ $('gameGrid').innerHTML=games.map(g=>`<button class="card ${selected&&g.id===selected.id?'selected':''}" data-id="${g.id}" style="--cardAccent:${g.accent};--glow:${g.accent}66"><div class="thumb ${g.thumb}">${thumbInner(g.id)}</div><div class="titleRow"><h2>${g.emoji} ${g.title}</h2><span class="year">${g.year}</span></div><div class="tag">${g.tag}</div><p class="desc">${g.desc}</p><span class="ready">READY</span></button>`).join(''); document.querySelectorAll('.card').forEach(b=>b.onclick=()=>{selected=games.find(g=>g.id===b.dataset.id); setAccent(); renderCards(); updateSelected(); openGamePrompt(); beep(620)}); }
...
function openInstructions(){closeGamePrompt();setAccent(); $('instTitle').textContent=`${selected.emoji} ${selected.title}`; $('instBrief').textContent=selected.brief; updateControlInstructions(); $('objectiveList').innerHTML=selected.objectives.map(o=>`<li>${o}</li>`).join(''); showScreen(inst); beep(330)}
