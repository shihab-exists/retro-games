...
let selected=null, settings={difficulty:'normal',sound:true,crt:true,visual:'glow'};
...
function renderCards(){ $('gameGrid').innerHTML=games.map(g=>`<button class="card ${g.id===selected.id?'selected':''}" data-id="${g.id}" style="--cardAccent:${g.accent};--glow:${g.accent}66"><div class="thumb ${g.thumb}">${thumbInner(g.id)}</div><div class="titleRow"><h2>${g.emoji} ${g.title}</h2><span class="year">${g.year}</span></div><div class="tag">${g.tag}</div><p class="desc">${g.desc}</p><span class="ready">READY</span></button>`).join(''); document.querySelectorAll('.card').forEach(b=>b.onclick=()=>{selected=games.find(g=>g.id===b.dataset.id); setAccent(); renderCards(); updateSelected(); beep(620)}); }
...
function showScreen(s){[menu,inst,play].forEach(x=>x.classList.add('hidden')); s.classList.remove('hidden')}
function openInstructions(){setAccent(); $('instTitle').textContent=`${selected.emoji} ${selected.title}`; $('instBrief').textContent=selected.brief; $('controlList').innerHTML=selected.controls.map(c=>`<div class="keyline"><span class="kbd">${c[0]}</span><br>${c[1]}</div>`).join(''); $('objectiveList').innerHTML=selected.objectives.map(o=>`<li>${o}</li>`).join(''); showScreen(inst); beep(330)}
$('randomBtn').onclick=()=>{selected=games[Math.floor(Math.random()*games.length)];setAccent();renderCards();updateSelected();beep(760)}; $('detailsBtn').onclick=openInstructions; $('startBtn').onclick=openInstructions; $('backMenuBtn').onclick=()=>{showScreen(menu);beep(220)}; $('playNowBtn').onclick=()=>startGame(); $('quitBtn').onclick=()=>quitGame(); $('pauseBtn').onclick=()=>togglePause(); $('resumeBtn').onclick=()=>togglePause();
$('difficulty').onchange=e=>settings.difficulty=e.target.value; $('soundToggle').onchange=e=>settings.sound=e.target.checked; $('crtToggle').onchange=e=>{settings.crt=e.target.checked;document.body.classList.toggle('scanlines',settings.crt)}; $('visualMode').onchange=e=>{settings.visual=e.target.value; document.body.style.filter=e.target.value==='high'?'contrast(1.18) saturate(1.25)':e.target.value==='clean'?'none':'saturate(1.08)'};
async function init(){
  try{
    const res=await fetch('/api/games');
    if(res.ok){ games=await res.json(); }
  }catch(e){ console.warn('Using local fallback game data because API is unavailable.', e); }
  selected=games[0];
...
function diffMult(){return settings.difficulty==='easy'?.78:settings.difficulty==='hard'?1.28:1}
function startGame(){showScreen(play); $('playTitle').textContent=`${selected.emoji} ${selected.title}`; $('pauseOverlay').innerHTML='<div class="box"><h3 class="pixel">PAUSED</h3><p>Press P or the Pause button to continue.</p><button id="resumeBtnInline" class="btn primary">Resume</button></div>'; $('pauseOverlay').classList.add('hidden'); setTimeout(()=>{const b=$('resumeBtnInline'); if(b)b.onclick=()=>togglePause();},0); paused=false; keys={}; if(engine&&engine.destroy)engine.destroy(); engine=makeEngine(selected.id); last=performance.now(); cancelAnimationFrame(raf); loop(last); beep(880,.08)}
function quitGame(){cancelAnimationFrame(raf); engine=null; showScreen(menu); beep(180,.08)}
...
function hud(score,lives,status){$('scoreHud').textContent='SCORE '+score; $('lifeHud').textContent='LIVES '+lives; $('statusHud').textContent=status||'RUNNING'}
function press(...arr){return arr.some(k=>keys[k]||keys[k.toLowerCase()])}
